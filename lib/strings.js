...
const parseTokens = (string, tokenMap) => {
  const maxTokenLength = greatest(Object.keys(tokenMap).map(token => token.length));
  const parsed = [];

  for (let i = 0; i < string.length; i += 1) {
    for (let j = 1; j <= maxTokenLength && i + j <= string.length; j += 1) {
      const slice = string.slice(i, i + j);
