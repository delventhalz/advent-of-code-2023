...
const getParser = (type, parseFn) => (binary) => {
...
const parseLiteral = getParser('LITERAL', (binary) => {
  let i = 0;
  let bits = '';
...
const getOperands = (binary, parseWhile) => {
  let length = 0;
  let operands = [];

  while (parseWhile({ length, operands })) {
...
const parseOperator = getParser('OPERATOR', (binary) => {
  const headerLength = binary[0] === '0' ? 15 + 1 : 11 + 1;

  const size = toDecimal(binary.slice(1, headerLength));
  const parseWhile = binary[0] === '0'
    ? ({ length }) => length < size
    : ({ operands }) => operands.length < size;

  const { length, operands } = getOperands(binary.slice(headerLength), parseWhile);
  return {
    operands,
...
const parsePacket = (binary) => {
  switch (toDecimal(binary.slice(3, 6))) {
    case 4:
...
const sumVersions = ({ version, operands = []}) => {
  return operands.reduce((sum, operand) => sum + sumVersions(operand), version);
};
