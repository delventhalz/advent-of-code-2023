...
const getVisible = (map, start, iterator) => {
  const visible = [start.join(',')];
  let tallest = map[ start[1] ][ start[0] ];

  iterator(map, start, (height, coords) => {
    if (height > tallest) {
      visible.push(coords.join(','));
      tallest = height;
    }
  });
...
module.exports = (_, rawInput) => {
  ...
  eachMatrix(map, (_, [x, y]) => {
    if (x === 0) {
      visible.push(...getVisible(map, [x, y], eachRight));
    } else if (x === lastX) {
      visible.push(...getVisible(map, [x, y], eachLeft));
    }

    if (y === 0) {
      visible.push(...getVisible(map, [x, y], eachDown));
    } else if (y === lastY) {
      visible.push(...getVisible(map, [x, y], eachUp));
    }
  });
