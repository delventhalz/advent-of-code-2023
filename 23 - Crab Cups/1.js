...
const move = (cups) => {
  // Mutating cups array with splice to remove three elements!
  ...
  if (destination < least(cups)) {
    destination = greatest(cups);
  }

  // Put removed cups back
  cups.splice(cups.indexOf(destination) + 1, 0, ...removed);

  // Shifting so current cup is always just the first one
  const oldCurrent = cups.shift();
  cups.push(oldCurrent);
};


module.exports = (inputs) => {
  const cups = inputs.split('').map(Number);
