...
const { matrixToString, mapMatrix } = require('../lib/arrays.js');
...
module.exports = (inputs) => {
  ...
  for (let y = 0; y < 50; y++) {
    outputs[y] = [];
    for (let x = 0; x < 50; x++) {
      outputs[y].push(getRunner(inputs, { quietIO: true })(x, y)[0]);
    }
  }

  console.log(matrixToString(mapMatrix(outputs, o => o === 1 ? '#' : '.')));
  return outputs.flat().filter((output) => output === 1).length;
};
