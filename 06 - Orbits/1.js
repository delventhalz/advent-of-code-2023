...
const getOrbitCounter = (orbitMap) => function countOrbits(satellite) {
  if (!orbitMap[satellite]) {
    return 0;
  }

  return 1 + countOrbits(orbitMap[satellite]);
};

module.exports = (inputs) => {
  const orbitMap = toOrbitMap(inputs);
  const orbitCounts = Object.keys(orbitMap).map(getOrbitCounter(orbitMap));
