...
// can be negative: 1101,100,-1,4,0 is a valid program (find 100 + -1, store
// the result in position 4). The TEST diagnostic program will start by
...
const getParamFetcher = (state) => (opCode, ...params) => {
  // Mode of "1" is "immediate", otherwise lookup param as normal
  const modes = String(opCode)
    .slice(0, -2)
    .split('')
    .reverse();


  return params.map((param, i) => modes[i] === '1' ? param : state[param]);
};

const getParamSlicer = (state) => (i, count) => state.slice(i + 1, i + 1 + count);


const getAdder = (state) => (x, y, outIndex) => {
  state[outIndex] = x + y;
};

const getMultiplier = (state) => (x, y, outIndex) => {
  state[outIndex] = x * y;
};

const getInputter = (state, inputVal) => (outIndex) => {
  console.log('input:', inputVal);
  state[outIndex] = inputVal;
};

const getOutputter = () => (x) => {
  console.log('output:', x);
}

const getRunner = (initialState) => (inputVal) => {
  const state = initialState.slice();

  const fetchParams = getParamFetcher(state);
  const sliceParams = getParamSlicer(state);

  const add = getAdder(state);
  const multiply = getMultiplier(state);
  const input = getInputter(state, inputVal);
  const output = getOutputter(state);

  for (let i = 0; i < state.length; i += 0) {
    const opCode = state[i];
    const op = Number(String(opCode).slice(-2));

    if (op === 1) {
      const params = fetchParams(opCode, ...sliceParams(i, 2));
      add(...params, state[i + 3]);
      i += 4;
    } else if (op === 2) {
      const params = fetchParams(opCode, ...sliceParams(i, 2));
      multiply(...params, state[i + 3]);
      i += 4;
    } else if (op === 3) {
      input(state[i + 1]);
      i += 2;
    } else if (op === 4) {
      const params = fetchParams(opCode, state[i + 1]);
      output(...params);
      i += 2;
    } else if (op === 99) {
      return 0;
