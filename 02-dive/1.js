...
module.exports = (inputs) => {
  ...
  const [laterals, verticals] = partition(commands, ([cmd]) => cmd === 'forward');

  const horizontal = sum(laterals.map(([_, val]) => val));
  const depth = sum(verticals.map(([cmd, val]) => cmd === 'up' ? -val : val));
