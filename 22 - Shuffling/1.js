...
// through 10006. The deck must be new - they're still in factory order, with 0
// on the top, then 1, then 2, and so on, all the way through to 10006 on the
// bottom.
...
const dealIntoStack = (deck) => deck.slice().reverse();
const cutNCards = (deck, n) => [...deck.slice(n), ...deck.slice(0, n)];
const dealWithN = (deck, n) => {
  const { length } = deck;
  const dealt = [];

  for (let i = 0; i < length; i++) {
    dealt[i * n % length] = deck[i];
  }

  return dealt;
};

const deal = (deck, instruction) => {
  if (instruction === 'deal into new stack') {
    return dealIntoStack(deck);
  }

  if (instruction.slice(0, 3) === 'cut') {
    return cutNCards(deck, Number(instruction.slice(4)));
  }

  if (instruction.slice(0, 19) === 'deal with increment') {
    return dealWithN(deck, Number(instruction.slice(20)));
  }
