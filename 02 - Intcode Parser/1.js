...
const getAdder = (arr) => (xIndex, yIndex, outIndex) => {
  arr[outIndex] = arr[xIndex] + arr[yIndex];
};

const getMultiplier = (arr) => (xIndex, yIndex, outIndex) => {
  arr[outIndex] = arr[xIndex] * arr[yIndex];
};
...
module.exports = (inputs) => {
  ...
  const add = getAdder(inputs);
  const multiply = getMultiplier(inputs);


  for (let i = 0; i < inputs.length; i += 4) {
    const opCode = inputs[i];
    const indexes = inputs.slice(i + 1, i + 4);
    if (opCode === 1) {
      add(...indexes);
    } else if (opCode === 2) {
      multiply(...indexes);
...
// Your puzzle answer was 3716250.
