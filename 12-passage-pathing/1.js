...
const traverse = (map, room, path, blocked) => {
  ...
  return map[room]
    .filter(connect => !nextBlocked.has(connect))
    .flatMap(connect => traverse(map, connect, nextPath, nextBlocked));
};


module.exports = (inputs) => {
  const map = mapConnections(inputs.map(connect => connect.split('-')));
  const paths = traverse(map, 'start', [], new Set());
  return paths.length;
};
