...
const parseRule = ([line]) => {
  const [_, ruleString] = line.split(': ');
  return or(ruleString.split(' or ').map(toPredicate));
};


module.exports = (inputs) => {
  const rules = inputs[0].map(parseRule);
  const nearbyTickets = inputs[2].slice(1);

  let errorRate = 0;

  for (const ticket of nearbyTickets) {
    const failed = ticket.filter(num => !rules.some(rule => rule(num)));
    errorRate += sum(failed);
  }
