...
const foldLeft = (dots, xFold) => {
  return dots.map(([x, y]) => x > xFold ? [x - xFold - 1, y] : [xFold - x - 1, y]);
}

// eslint-disable-next-line no-unused-vars
const foldUp = (dots, yFold) => {
  return dots.map(([x, y]) => y < yFold ? [x, y] : [x, yFold - (y - yFold)]);
}

module.exports = (_, rawInput) => {
  const [start, end] = rawInput.split('\n\n');
  const dots = parseDots(start);
  const folds = parseFolds(end);

  return uniq(foldLeft(dots, folds[0].x).map(String)).length;
};
