...
const mapRowToCoords = (mapRow, y) => mapRow
  .split('')
  .flatMap((point, i) => point === '#' ? i : [])
  .map(x => [x, y]);

const mapToCoords = (mapRows) => mapRows.flatMap(mapRowToCoords);

const getLaserAngle = (x1, y1, x2, y2) => angles.normalize(
  angles.fromSlope([x1, y1], [x2, y2]) + 90
);

const isSegmentClear = (x1, y1, x2, y2, points) => {
  for (const [px, py] of points) {
    if (px === x1 && py === y1) continue;
    if (px === x2 && py === y2) continue;
    if (distanceToLineSegment(x1, y1, x2, y2, px, py) === 0) {
      return false;
    }
  }
...
const laserRotation = (x1, y1, targets, previousHits) => {
  const misses = [];
  const hits = [];

  for (const [x2, y2] of targets) {
    if (isSegmentClear(x1, y1, x2, y2, targets)) {
      hits.push([x2, y2]);
    } else {
      misses.push([x2, y2]);
    }
  ...
  hits.sort((a, b) => getLaserAngle(x1, y1, ...a) - getLaserAngle(x1, y1, ...b));

  return [misses, previousHits.concat(hits)];
};

const getSortedLaserHits = (x1, y1, coords) => {
  let targets = coords.filter(([x, y]) => x !== x1 || y !== y1);
  let hits = [];

  while(targets.length > 0) {
    ([targets, hits] = laserRotation(x1, y1, targets, hits));
  }
...
module.exports = (inputs) => {
  const coords = mapToCoords(inputs);
  const hits = getSortedLaserHits(20, 18, coords)
  return hits[199];  // 200th
};
