...
const countVisibles = (coords) => {
  ...
  for (const [x1, y1] of coords) {
    ...
    for (const [x2, y2] of coords) {
      ...
      if (isSegmentClear(x1, y1, x2, y2, coords)) {
        count++;
      }
    }
    counts.push({ count, x: x1, y: y1 });
  }
...
module.exports = (inputs) => {
  ...
  return greatest(countVisibles(coords), ({ count }) => count);
};


// Your puzzle answer was 280.
// (Note: the coordinates were 20,18)
