...
module.exports = (_, rawInput) => {
  ...
  const possibilities = getPossibleAllergens(recipes);

  const ingredients = recipes.flatMap(recipe => recipe[0]);
  const possibleSet = new Set(Object.values(possibilities).flat());

  return count(ingredients, ingredient => !possibleSet.has(ingredient));
};

// Your puzzle answer was 2595.
