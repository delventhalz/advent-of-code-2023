...
const { snailSum, snailMagnitude } = require('./snails.js');


module.exports = (_, rawInput) => {
  const snailNumbers = rawInput.split('\n');
  let largest = 0;

  for (const snailNumA of snailNumbers) {
    for (const snailNumB of snailNumbers) {
      if (snailNumA !== snailNumB) {
        const sum = snailSum(snailNumA, snailNumB);
        const magnitude = snailMagnitude(sum);
