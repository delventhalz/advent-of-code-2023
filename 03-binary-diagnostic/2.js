...
const findMost = (inputs, i) => {
  if (inputs.length <= 1) {
    return inputs[0];
  }
  const zeroCount = count(inputs, input => input[i] === '0');

  if (zeroCount > inputs.length / 2) {
    return findMost(inputs.filter(input => input[i] === '0'), i + 1);
  } else {
    return findMost(inputs.filter(input => input[i] === '1'), i + 1);
  }
};

const findLeast = (inputs, i) => {
  if (inputs.length <= 1) {
    return inputs[0];
  }
  ...
  if (zeroCount > inputs.length / 2) {
    return findLeast(inputs.filter(input => input[i] === '1'), i + 1);
  } else {
    return findLeast(inputs.filter(input => input[i] === '0'), i + 1);
  }
};
...
module.exports = (_, rawInput) => {
  ...
  const most = findMost(inputs, 0);
  const least = findLeast(inputs, 0);
